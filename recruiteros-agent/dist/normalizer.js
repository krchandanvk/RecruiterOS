...
exports.normalizeScrapedData = normalizeScrapedData;
/**
 * Normalization layer to clean raw parsed strings, format dates, and structure types.
 */
function normalizeScrapedData(raw) {
    return {
        candidate: {
        ...
        education: (raw.education || []).map((edu) => {
            ...
            if (edu.startYear) {
                ...
                if (!isNaN(val) && val > 1950 && val < 2050)
                    startYear = val;
            }
            if (edu.endYear) {
        ...
        skills: (raw.skills || []).map((s) => cleanText(s)).filter((s) => s.length > 0),
    };
}
/**
