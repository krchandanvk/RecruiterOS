...
exports.processCandidateWithAI = processCandidateWithAI;
...
const getGenAIClient = () => {
    ...
    return new generative_ai_1.GoogleGenerativeAI(config_js_1.config.geminiApiKey);
};
/**
 * Invokes Gemini AI to synthesize candidate profile data, categorizes skills,
 * and generates scores/summaries.
 */
async function processCandidateWithAI(normalizedData) {
...
`;
        ...
        if (parsed.skillsCategorized) {
            parsed.skillsCategorized = parsed.skillsCategorized.map((s) => {
 ...
 */
function generateFallbackMockValues(normalizedData) {
    ...
    const skillsCategorized = rawSkills.map((sk) => {
    ...
    if (normalizedData.candidate.about)
        completeness += 10;
    if (normalizedData.candidate.profilePhotoUrl)
        completeness += 10;
    if (normalizedData.experiences.length > 0)
        completeness += 20;
