...
async function scrapeLinkedInProfile(page, profileUrl) {
    ...
    const fullName = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.fullName) || "Unknown Name";
    const headline = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.headline);
    const rawLocation = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.location);
    const about = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.about);
    const profilePhotoUrl = await (0, selectors_js_1.queryFallbackAttr)(page, selectors_js_1.SELECTORS.profilePhoto, "src");
    // Scrape followers / connections
    const rawFollowers = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.followers);
    let followersCount = null;
    if (rawFollowers) {
        const numMatch = rawFollowers.replace(/,/g, "").match(/\d+/);
        if (numMatch)
            followersCount = parseInt(numMatch[0]);
    }
    const rawConnections = await (0, selectors_js_1.queryFallbackText)(page, selectors_js_1.SELECTORS.connections);
    let connectionsCount = null;
    if (rawConnections) {
        const numMatch = rawConnections.replace(/,/g, "").match(/\d+/);
        if (numMatch)
            connectionsCount = parseInt(numMatch[0]);
    }
    // Open to work tag checks
    let openToWork = false;
    for (const selector of selectors_js_1.SELECTORS.openToWork) {
...
async function scrapeExperiencesExhaustive(page, profileUrl) {
...
async function scrapeEducationExhaustive(page, profileUrl) {
    const educationList = [];
    try {
...
async function scrapeCertificationsExhaustive(page, profileUrl) {
    const certsList = [];
    try {
...
async function scrapeProjects(page, profileUrl) {
    const projectsList = [];
    try {
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            if (text && text.trim() && text.length < 50 && !skillsList.includes(text.trim())) {
                ...
                if (!lowercase.includes("endorsed") &&
