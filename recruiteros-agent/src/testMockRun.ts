...
async function runMockIntegrationTest() {
  ...
  console.log(`API Target URL: ${config.apiBaseUrl}`);
  console.log("==================================================================");
  ...
  let job: any = null;
  try {
    const jobResponse = await apiClient.fetchNextJob();
  ...
  const normalized = normalizeScrapedData(mockRawScraped);
  ...
  const aiOutput = {
    ...
    skillsCategorized: [
      { skillName: "Node.js", category: "Backend" as const },
      { skillName: "TypeScript", category: "Frontend" as const },
      { skillName: "React", category: "Frontend" as const },
      { skillName: "Next.js", category: "Frontend" as const },
      { skillName: "AWS", category: "Cloud" as const },
      { skillName: "PostgreSQL", category: "Database" as const },
      { skillName: "Docker", category: "DevOps" as const },
      { skillName: "Jest", category: "Testing" as const },
      { skillName: "GraphQL", category: "Backend" as const }
    ],
    candidateScore: 92,
  ...
  try {
    ...
    const submitOk = await apiClient.submitJobSuccess(payload);
    if (!submitOk) {
  ...
  try {
    ...
    const johnRecord = verifyData.candidates?.find((c: any) => c.linkedinUrl === testProfileUrl);
    ...
    console.log(`Emails Scraped:  ${johnRecord.emails.map((e: any) => e.email).join(", ")}`);
    console.log(`Skills:         ${johnRecord.skills.map((s: any) => s.skillName).join(", ")}`);
    console.log("------------------------------------------------------------------");
  } catch (err: any) {
